...
(function () {
  ...
  // Open the admin panel as an interactive panel inside OBS Studio.
  // This lets the user configure the plugin without ever leaving OBS.
  const openInObsBtn = $("open-in-obs-btn");
  if (openInObsBtn) {
    openInObsBtn.addEventListener("click", async () => {
      openInObsBtn.disabled = true;
      openInObsBtn.textContent = "请求中…";
      try {
        const r = await fetch("/api/obs/open-admin", { method: "POST" });
        const j = await r.json();
        if (!r.ok) throw new Error(j.error || "请求失败");
        openInObsBtn.textContent = "已在 OBS 中打开";
        setTimeout(() => { openInObsBtn.textContent = "在 OBS 中打开设置面板"; }, 2500);
      } catch (e) {
        openInObsBtn.textContent = `失败: ${e.message}`;
        setTimeout(() => { openInObsBtn.textContent = "在 OBS 中打开设置面板"; }, 3000);
      } finally {
        openInObsBtn.disabled = false;
      }
    });
  }
