...
async fn try_connect(
    ...
    // Best-effort: auto-create a hidden browser source that hosts the
    // admin panel. The user can open it as an in-OBS interactive panel
    // ("Interact" button on the source) and configure the plugin without
    // ever leaving OBS Studio.
...
/// Create (or update) the hidden `browser_source` that hosts the admin
/// panel inside OBS. It is created disabled so it never appears in the
/// recording / stream — the user opens it through the
/// "OpenInputInteract" path on demand.
async fn ws_create_admin_browser_source<W>(write_half: &mut W, url: &str) -> Result<()>
where
    W: futures::Sink<Message, Error = tokio_tungstenite::tungstenite::Error> + Unpin,
{
    // 1) Make sure the input itself exists. We use the i18n-safe scene
    //    name "Current Scene" so it works on both Chinese and English OBS
    //    installs.
    let create = serde_json::json!({
        "op": 6,
        "d": {
    ...
    // The CreateInput call may fail (e.g. the user is currently on a
    // different scene, or the input already exists with different
    // settings). We treat any error as non-fatal and continue to the
    // SetInputSettings call below, which creates the input if missing.
    let _ = write_half.send(Message::Text(create.to_string().into())).await;

    // 2) Force the input settings. This also works as a no-op if the
    //    input already exists with the same settings.
    let update = serde_json::json!({
        "op": 6,
        "d": {
            "requestType": "SetInputSettings",
...
/// Ask OBS to open the admin browser source as an interactive panel
/// inside the OBS main window. This is the equivalent of the user
/// right-clicking the source in OBS and choosing "Interact".
