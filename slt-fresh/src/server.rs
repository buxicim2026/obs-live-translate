...
fn build_router(state: Arc<AppState>, static_dir: PathBuf) -> Router {
    let api = Router::new()
        ...
        .route("/api/obs/open-admin", post(open_admin_in_obs))
...
async fn open_admin_in_obs(State(state): State<Arc<AppState>>) -> Response {
    let tx_opt = state.obs_cmd_tx.lock().clone();
    let Some(tx) = tx_opt else {
        return (
            StatusCode::SERVICE_UNAVAILABLE,
            axum::Json(serde_json::json!({
                "error": "OBS WebSocket is not yet connected"
            })),
        )
            .into_response();
    };
    if !state.status.read().obs_connected {
        return (
            StatusCode::SERVICE_UNAVAILABLE,
            axum::Json(serde_json::json!({
                "error": "OBS Studio is not currently connected"
            })),
        )
            .into_response();
    }
    match tx.send(crate::obs::ObsCommand::OpenAdminInObs).await {
